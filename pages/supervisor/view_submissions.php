...
// Validate that the project belongs to this supervisor
try {
    $stmt = $pdo->prepare("
        SELECT 
            p.ProjectID,
...
// Handle submission review with grading
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && in_array($_POST['action'], ['accept', 'reject', 'grade'])) {
    ...
    if ($submissionID <= 0) {
        ...
        try {
            ...
            // First, check the maximum marks for this milestone
            $stmt = $pdo->prepare("
                SELECT 
                    m.MaxMarks
                FROM 
            ...
            ");
            $stmt->execute([':submissionID' => $submissionID]);
            $maxMarks = $stmt->fetchColumn();
            
            // Validate marks against max marks
            if ($marks !== null && ($marks < 0 || $marks > $maxMarks)) {
                throw new Exception("Marks must be between 0 and the maximum marks ($maxMarks).");
            }
            
            // Update submission review status and marks
            $reviewStatus = ($action === 'accept') ? 'Accepted' : (($action === 'reject') ? 'Rejected' : 'Graded');
            $stmt = $pdo->prepare("
                UPDATE Submissions 
                SET 
            ...
            ");
            $stmt->execute([':submissionID' => $submissionID]);
            $milestoneTitle = $stmt->fetchColumn();
            
            // Add to project history
            $historyAction = "Submission " . $reviewStatus;
            if ($marks !== null) {
                $historyAction .= " - Marks: $marks / $maxMarks";
            }
            $stmt = $pdo->prepare("
            ...
            // If the submission is for a group project, create notifications for all group members
            $stmt = $pdo->prepare("
                SELECT 
                    p.GroupID
                FROM 
            ...
            if ($groupID) {
                $stmt = $pdo->prepare("
                    SELECT StudentID FROM StudentGroups WHERE GroupID = :groupID AND Status = 'Active'
                ");
                ...
                $notificationTitle = "Submission {$reviewStatus}";
                $notificationMessage = "Your submission for '{$milestoneTitle}' has been {$reviewStatus} by {$supervisorName}.";
                if ($marks !== null) {
                    $notificationMessage .= " Marks received: {$marks}/{$maxMarks}";
                }
                
                // Insert notifications for all students in the group
            ...
            header("Location: view_submission.php?submission_id=" . $submissionID . "&success=1");
            exit();
...
if (isset($_GET['success']) && $_GET['success'] == '1') {
    $success = "Submission has been reviewed successfully.";
}
...
if (empty($error) && !empty($projectData['GroupID'])) {
    try {
        $stmt = $pdo->prepare("
            SELECT 
                u.UserID,
                        ...
                        <?php foreach ($submissions as $index => $submission): ?>
                            <div x-show="openTab === <?php echo $index; ?>">
                                ...
                                <?php if ($submission['ReviewStatus'] === 'Pending'): ?>
                                    <div x-data="{ showAcceptForm: false, showRejectForm: false, showGradeForm: false }">
                                        <h3 class="font-semibold text-lg mb-3">Review Submission</h3>
                                        <div class="flex flex-wrap gap-4 mb-4">
                                            <button 
                                                @click="showAcceptForm = true; showRejectForm = false; showGradeForm = false"
                                                class="bg-green-500 hover:bg-green-600 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition duration-200"
                                            >
                                                Accept Submission
                                            </button>
                                            <button 
                                                @click="showRejectForm = true; showAcceptForm = false; showGradeForm = false"
                                                class="bg-red-500 hover:bg-red-600 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition duration-200"
                                            >
                                                Reject Submission
                                            </button>
                                            <button 
                                                @click="showGradeForm = true; showAcceptForm = false; showRejectForm = false"
                                                class="bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg shadow-sm transition duration-200"
                                            >
                                                Grade Submission
                                            </button>
                                        </div>
                                        ...
                                        <!-- Grade Form -->
                                        <div x-show="showGradeForm" class="bg-gray-50 rounded-lg p-4 mb-4">
                                            <form method="POST" class="space-y-4">
                                                <input type="hidden" name="submission_id" value="<?php echo $submission['SubmissionID']; ?>">
                                                <input type="hidden" name="action" value="grade">
                                                
                                                <div>
                                                    <label for="marks" class="block text-sm font-medium text-gray-700 mb-1">Marks (out of <?php echo $submission['MaxMarks']; ?>)</label>
                                                    <input 
                                                        type="number" 
                                                        id="marks" 
                                                        name="marks" 
                                                ...
                                                <div>
                                                    <label for="grade-feedback" class="block text-sm font-medium text-gray-700 mb-1">Feedback and Comments</label>
                                                    <textarea 
                                                        id="grade-feedback" 
                                                        name="feedback" 
                                                        rows="4" 
                                                        class="w-full px-4 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                                                        placeholder="Provide detailed feedback on the submission quality and areas for improvement..."
                                                        required
                                                    ></textarea>
                                                </div>
                                    ...
                                    <div class="bg-gray-50 rounded-lg p-4 mb-4">
                                        ...
                                        >
                                            <p class="text-sm font-medium mb-2">
                                                <?php 
                                                    if ($submission['ReviewStatus'] === 'Accepted') {
                                                        echo 'Acceptance Feedback:';
                                                    } elseif ($submission['ReviewStatus'] === 'Rejected') {
                                                        echo 'Rejection Reason:';
                                                    } else {
                                                        echo 'Grading Feedback:';
                                                    }
                                                ?>
                                            </p>
